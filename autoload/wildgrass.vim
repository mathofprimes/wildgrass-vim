...
function! wildgrass#generate_palette(dark, light, contrast)
    ...
    for bf in ['0', '1', '2', '3']
        ...
        let palette['bg' . bf] = '#' . printf('%02X', bg[0] * rgb_bg[0] + count_bg)
                                   \ . printf('%02X', bg[0] * rgb_bg[1] + count_bg)
                                   \ . printf('%02X', bg[0] * rgb_bg[2] + count_bg)
        
        let palette['fg' . bf] = '#' . printf('%02X', fg[1] * rgb_fg[0] + count_fg)
                                   \ . printf('%02X', fg[1] * rgb_fg[1] + count_fg)
                                   \ . printf('%02X', fg[1] * rgb_fg[2] + count_fg)
    endfor

    for [name, rgb_sn] in items(rgb)
        let palette[name] = '#' . printf('%02X', sn[2] * rgb_sn[0] + csn[rgb_sn[3]])
                              \ . printf('%02X', sn[2] * rgb_sn[1] + csn[rgb_sn[3]])
                              \ . printf('%02X', sn[2] * rgb_sn[2] + csn[rgb_sn[3]])
    endfor
